<template>
    <div>
    <canvas id="sentimientosFonasa" width="300" height="200"></canvas>
    </div>
</template>
...
export default {
    data() {
        return {
            ...
            chartData:{
                type:'bar',
                data: {
                    labels:['Sentimientos respecto a Fonasa'],
                    datasets: [
                        {
                            label: 'Acuerdo',
                            data: [],
    ...
    methods: {
        createChart(chartId, chartData) {
            const url = 'http://localhost:8081/fondoDeSalud/getFonasa';
    ...
    mounted() {
        this.createChart('sentimientosFonasa', this.chartData);
    },
}
</script>